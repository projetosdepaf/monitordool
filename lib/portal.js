...
// "DECRETOS NUMERADOS" e "DECRETOS SIMPLES" não ficam dentro da pasta da
// Polícia Civil (são assinados pelo Governador, então ficam soltos, junto de
// decretos de todas as secretarias) — mas é exatamente ali que saem
// nomeação/exoneração de cargo comissionado (DAS), incluindo da DEPAF. Cada
// edição normalmente tem UM documento só nessas pastas, juntando dezenas de
// atos de secretarias diferentes num texto só — por isso a gente sempre
// abre essas duas pastas (mesmo sem "Polícia Civil" no nome) e depois separa
// ato por ato, mantendo só os que falam de Polícia Civil/DEPAF. Descoberto
// junto com a Cíntia em 25/08/2026, comparando com o que ela lembrava de
// verdade ter saído no dia 22/08/2026 (a nomeação dela, a do Ednei e duas
// exonerações) e que o robô estava perdendo.
const PASTAS_LOTE_NORM = ['decretos numerados', 'decretos simples'];
function ehPastaLote(nomePasta) {
  return PASTAS_LOTE_NORM.includes(normalize(nomePasta));
}
