...
const { classificarAssunto } = require('./assunto');
...
function avaliarConteudo(texto, depafTerms) {
  ...
  const { assunto, motivo } = classificarAssunto(texto);
  return {
    isDepafMention: Boolean(termoDepaf),
    matchedTerm: termoDepaf || 'Polícia Civil da Bahia',
    // movementType diz que ESPÉCIE de documento é; assunto/motivo dizem do que
    // ele TRATA. Os dois convivem: o painel filtra por assunto/motivo, mas o
    // movementType continua gravado (ver lib/assunto.js).
...
function atoRelevante(texto, termosAmplos) {
  const normTexto = normalize(texto);
  if (termosAmplos.some((t) => t && normTexto.includes(normalize(t)))) return true;
  // Aqui é o único ponto em que uma publicação pode ser DESCARTADA por causa da
  // grafia: um ato solto dentro de um decreto em lote que cite só a DEPAF
  // abreviada, sem escrever "Polícia Civil". Nos outros casos a captura vem da
  // pasta do órgão no sumário, que já é "Polícia Civil".
  return Boolean(acharMencaoDepaf(texto));
}
