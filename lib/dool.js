...
const { descobrirPaginas, lerProporcao, salvarProporcao } = require('./pagina');
...
async function processarEdicao(edicaoInfo, { log = () => {}, salvarSumario } = {}) {
  ...
  const semPagina = resultados.filter((r) => !(r.page > 0)).length;
  if (semPagina && DESCOBRIR_PAGINA) {
    log(`${semPagina} publicação(ões) sem página no sumário; procurando no PDF da edição.`);
    try {
